...
def f(x):
    q = -0.273*x
    volume = -153.8461*math.exp(q) - 563.539*math.exp(q) + ((5*math.sin(2*math.pi*x))/math.pi)+3900
    return volume

#Definisi fungsi volume terhadap waktu untuk x1 (Vi)
def funcOne(dataXValOne):
    q = -0.273*x
    volume = -153.8461*math.exp(q) - 563.539*math.exp(q) + ((5*math.sin(2*math.pi*x))/math.pi)+3900
    return volume

#Definisi fungsi discharge terhadap besar volume (Di)
...
#Definisi total function TF untuk besar volume aktual (yi)
